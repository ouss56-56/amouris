  ...
  const handleDownloadSales = () => {
    ...
    setTimeout(() => {
        const data = orders.map(o => ({
            ...
            "Client": o.guest_first_name ? `${o.guest_first_name} ${o.guest_last_name}` : 'Client Enregistré',
            ...
            "Articles": o.items.map(i => `${i.product_name_fr} (${i.quantity_grams || i.quantity_units})`).join(', ')
        }))
  ...
  return (
    <div className="space-y-12 pb-20">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-8">
        <div>
           <h1 className="font-serif text-4xl text-emerald-950 mb-2 font-bolditalic">Bibliothèque de Rapports</h1>
        ...
        {reports.map((report) => (
          <div key={report.id} className="bg-white p-10 rounded-[3rem] border border-emerald-950/5 shadow-2xl shadow-emerald-950/5 flex flex-col justify-between group hover:-translate-y-2 transition-all duration-500">
             <div>
                ...
                <h3 className="text-2xl font-serif font-bold text-emerald-950 mb-4">{report.title}</h3>
      ...
      <section className="bg-neutral-900 rounded-[3rem] p-12 text-white/40 flex flex-col md:flex-row items-center gap-8 border border-white/5 shadow-2xl overflow-hidden relative group">
         ...
         <div className="flex-1 text-center md:text-left">
            <p className="text-white text-lg font-serif mb-1">Rapports temps réel</p>
