...
export default function AdminTagsPage() {
  const { tags, fetchTags, deleteTag, updateTag, isLoading } = useTagsStore()
  ...
  const toggleHomepage = async (tag: Tag) => {
  ...
  return (
    <div className="space-y-12 pb-20">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-8">
        <div>
           <h1 className="font-serif text-4xl text-emerald-950 mb-2">Les Wossoum</h1>
           <p className="text-[10px] font-black uppercase tracking-[0.4em] text-[#C9A84C]">Tags, Labels et Sections Homepage</p>
        </div>
        <button 
          onClick={handleAdd}
          className="bg-emerald-900 text-white px-8 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-2xl shadow-emerald-900/20 hover:scale-[1.02] active:scale-95 transition-all flex items-center gap-3"
        >
      ...
      {isLoading && tags.length === 0 ? (
        <div className="py-32 flex justify-center">
          <Loader2 className="animate-spin text-emerald-950" size={40} />
        </div>
      ) : (
            ...
            <table className="w-full border-collapse">
              ...
              <tbody className="divide-y divide-emerald-950/5">
                <AnimatePresence mode="popLayout">
                  {filtered.map((tag) => {
                    const count = products.filter(p => p.tag_ids?.includes(tag.id)).length
                    return (
                      <motion.tr 
                        layout
                        key={tag.id}
                      ...
                      >
                        <td className="px-10 py-6">
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <p className="font-serif text-lg text-emerald-950">{tag.name_fr}</p>
                        ...
                        <td className="px-10 py-6">
                           <button 
                             ...
                             className={`flex items-center gap-2 px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${tag.show_on_homepage ? 'bg-emerald-600 text-white' : 'bg-neutral-100 text-neutral-400'}`}
                           >
                             <Home size={12} />
